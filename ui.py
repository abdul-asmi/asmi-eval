...
<header>
  ...
  <div class="header-text">
    <div class="header-title">Asmi Testing <span style="font-size:0.8rem;color:#a78bfa;font-weight:400;margin-left:8px;vertical-align:middle;border:1px solid #7c3aed;padding:2px 6px;border-radius:4px;background:rgba(124,58,237,0.1)">Antigravity Enabled</span></div>
  </div>
</header>
...
function _sortedCategories(cats) {
  ...
  const currentOrder = _categoryOrderFromTests(tests).filter(c => set.has(c));
  const ordered = [];
  currentOrder.forEach(c => { if (!ordered.includes(c)) ordered.push(c); });
  KNOWN_CAT_ORDER.forEach(c => { if (set.has(c) && !ordered.includes(c)) ordered.push(c); });
